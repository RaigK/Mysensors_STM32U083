...
# Upload to device (requires ST-Link)
pio run -t upload
...
- `RTC_WKUP_IRQn=RTC_TAMP_IRQn` - U0 combines RTC wake-up into TAMP interrupt vector
...
1. Initialize RTC with LSE (32.768 kHz) or LSI fallback
2. Configure RTC wake-up timer (up to ~18 hours with CK_SPRE clock)
3. Disable peripheral clocks (SPI1, I2C1/2, ADC) before entering STOP
4. Enter STOP mode with low-power regulator
