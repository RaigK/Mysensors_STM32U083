...
# Alternative serial capture: 90-second window to stdout (survives MCU resets
# that disconnect pio device monitor). Requires pyserial.
...
**Custom OpenOCD target**: `stm32u0x.cfg` at the project root is hand-written (PlatformIO ships no built-in target for STM32U0 — it is derived from the STM32L0 config with the STM32L4x flash driver). `platformio.ini` references it via `board_debug.openocd_target = stm32u0x`, so PlatformIO's debug flow picks it up automatically. Standalone use: `openocd -f stm32u0x.cfg`. The examine-end event enables low-power debug mode via DBGMCU.

**Scratch / untracked files at the repo root that are NOT canonical sources** (ignore when reasoning about the project): `current_log.tsv` (output of `tools/read_serial.py`), `debug_hardfault.gdb` and `debug_session.gdb` (one-off GDB scripts), `run_debug.sh` (developer shortcut), `platformio - Copy.ini` (manual backup), and `nul` (Windows artifact from a redirect typo — safe to delete).
...
**MySensors.h `#include`s `MyHwSTM32.cpp` directly** (line 84) — it is not compiled as a separate library object. PlatformIO compiles the copy under `.pio/libdeps/<env>/MySensors/`, not the one in `lib/MySensors_patch/`. The `lib/MySensors_patch/` copy is the canonical source.

**`lib/MySensors_patch/library.json` is decorative**: PlatformIO LDF does not actually load `MySensors_patch` as a library in either env (verify with `pio run -e <env> -v` — patch is absent from the dependency tree). The `"frameworks": "arduino"` field, `srcFilter`, and the `-include hal/architecture/STM32/MyHwSTM32.h` flag inside it never apply. The patches only take effect through the manual file copy below.

**After every edit to a patched file, sync it to BOTH libdeps directories** (cube and Arduino envs each have their own copy):

```bash
for env in stm32u083 stm32u083_cube; do
  cp 'C:/claude/Mysensors_STM32U083/lib/MySensors_patch/hal/architecture/STM32/MyHwSTM32.cpp' \
     "C:/claude/Mysensors_STM32U083/.pio/libdeps/$env/MySensors/hal/architecture/STM32/MyHwSTM32.cpp"
  cp 'C:/claude/Mysensors_STM32U083/lib/MySensors_patch/hal/architecture/STM32/MyHwSTM32.h' \
     "C:/claude/Mysensors_STM32U083/.pio/libdeps/$env/MySensors/hal/architecture/STM32/MyHwSTM32.h"
done
```

**Foot-guns to remember**:
- The `.pio/libdeps/*/` copies are regenerated by `pio pkg install` (or after deleting `.pio/`), reverting the cube/Arduino libdeps to upstream MySensors. You must re-run the sync after any libdeps refresh.
- **Cube env was running unpatched upstream MySensors HAL until 2026-05** — the upstream `hwSleepInternal` calls `SystemClock_Config()` *before* `HAL_ResumeTick()`, which spinloops forever because `HAL_RCC_*Config` uses the frozen `HAL_GetTick()` for its timeouts. Symptom: device transmits once, then hangs after `Sleeping Xs...` with no `==WAKE==`. If the cube env ever regresses to this, the libdeps copy has been clobbered.
- **The cube env needs the arduino_compat STM32RTC shim to match the patch's STM32RTC API surface**. If the patch grows new calls (e.g. `rtc.disableAlarm(STM32RTC::ALARM_B)` or `rtc.getDate()`), `lib/arduino_compat/src/STM32RTC.{h,cpp}` must be extended in lockstep — the linker will name the missing symbol. Current shim covers: `LSI_CLOCK`, `ALARM_A`/`ALARM_B`, `setClockSource`, `begin`, `getHandle`, `getHours/Minutes/Seconds`, `attachInterrupt`, `disableAlarm`. The shim also owns the strong `RTC_TAMP_IRQHandler` and `HAL_RTC_AlarmAEventCallback` for the cube env (Arduino env gets these from STM32duino RTC instead).
...
- **`[env:stm32u083_cube]`** — bare-metal port (cubeu0-port branch). **The env declares no `framework =` at all** (not `framework = none`); together with `lib_compat_mode = off` this lets LDF resolve MySensors even though its `library.json` declares Arduino as required. Uses vendored STM32CubeU0 HAL (`lib/STM32CubeU0/`) + a minimal Arduino API shim (`lib/arduino_compat/`) layered over it. Custom linker script at `ldscripts/STM32U083xx_FLASH.ld`. `build_src_filter = -<*> +<main.cpp>` so only `src/main.cpp` is compiled (other helper mains were removed in commit e6d4f57). `lib_ignore = STM32duino RTC` is required because MySensors lists STM32duino RTC as a transitive dependency — without the ignore, LDF pulls it in and its Arduino-only includes fail to compile.
...
3. Call `hwPrepareSleep()`: force `DBGMCU->CR = 0` (unconditional, see STM32U0-specific notes below) → arm `EXTI->IMR1` bit 28 → set `EIWUL` → enable `RTC_TAMP_IRQn` in NVIC → **print diagnostics** → `hwConfigureGpioLowPower()` → disable peripheral clocks (SPI1, I2C1, I2C2, ADC). **All `Serial.print` diagnostics must appear before `hwConfigureGpioLowPower()`** — after that call PA2 (UART TX) goes to analog mode and all serial output is silently lost.
...
- **On STM32U0, EXTI28 is the RTC Alarm A wakeup line in Stop mode** — `RTC_EXTI_LINE_ALARM_EVENT = EXTI_IMR1_IM28` (same line as WUT). EXTI28 is a "direct line": no RTSR/FTSR edge configuration is possible; `IMR28=1` arms the interrupt path. Wake is taken on `RTC_TAMP_IRQn` (IRQ 2) — this is what the `RTC_WKUP_IRQn=RTC_TAMP_IRQn` build flag aliases. `hwPrepareSleep()` calls `NVIC_EnableIRQ(RTC_TAMP_IRQn)` defensively in case STM32duino RTC's init hasn't already done so. The handler itself (`RTC_TAMP_IRQHandler`) is provided as a strong symbol by the STM32duino RTC library — do NOT redefine it in `MyHwSTM32.cpp`. Stop mode entry uses **WFI**, not WFE (matches ST's `PWR_STOP2_RTC` reference example; WFE would additionally require `EMR28=1` for EXTI events). **Do NOT clear IMR28 before Stop mode** — it also disables the Alarm A interrupt, preventing wake-up.
...
- DBGMCU handling in `hwPrepareSleep()` **forces `DBGMCU->CR = 0` regardless of `MY_DEBUG`**. The code comment notes that enabling DBG-in-Stop on STM32U0 keeps clocks running and was observed to interfere with WUT wakeup; the SWD upload path is preserved through the 5-second boot window in `before()` (or Under-Reset upload), not via debug-in-stop. On STM32U0 specifically, `__HAL_RCC_DBGMCU_CLK_ENABLE()` must be called before any `DBGMCU` register write takes effect (unlike most other STM32 families).
...
- **2.11.1 SPI BSY stays high on disable in master transmit mode** (SPI, workaround: disable when TXE=1 and BSY=0): If SPI1 is disabled while TXE is low (data register full), BSY may stay high. The current `hwPrepareSleep()` does **not** poll TXE/BSY before calling `__HAL_RCC_SPI1_CLK_DISABLE()` — it gates the SPI clock directly. This is acceptable because the RFM69 driver completes its transactions (and asserts CS high) before `sleep()` is called; if a future change introduces SPI activity closer to the sleep entry point, add a TXE poll before the clock-disable.
